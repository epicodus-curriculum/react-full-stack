...
With that brief introduction in mind, let's implement a `useReducer()` hook. For this next practice, we'll revisit the `intro-to-hooks` application that we built [when we first learned how to use the `useState()` and `useEffect()` hooks](https://new.learnhowtoprogram.com/lessons/introduction-to-hooks-with-the-usestate-hook), and we'll refactor the `Counter` component we created to use a `useReducer()` hook.
